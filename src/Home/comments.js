...
const commentPosted=async()=>{
  ...
  const apiUrl="https://venkatsai.onrender.com/comments"
  const options={
    method:"POST",
    headers:{
        ...
        <div className='comments-model-bg-container'>
            ...
            <SwipeableViews enableMouseEvents index={activeIndex} onChangeIndex={handleSlideChange}>
              {thumbnails.map(eachImage=>{
                <img src={`https://venkatsai.onrender.com/${eachImage}`} className="descktop-comment-image" alt=""/>
                
                if(eachImage.split(".")[1]=="mp4"){
                  return(
                    <>
                     <div className='post-count-div'> <p className='post-count'>{`${activeIndex+1}/${thumbnails.length}`}</p></div>
                  <video  src={`https://venkatsai.onrender.com/${eachImage}`} autoPlay loop muted  className="descktop-comment-image"/>
                  </>)
                }
                else{
                  return(
                  <>
                  <img alt="" src={`https://venkatsai.onrender.com/${eachImage}`} className="descktop-comment-image" />
                  <div className='post-count-div'> <p className='post-count'>{`${activeIndex+1}/${thumbnails.length}`}</p></div>
                </>)
                }
