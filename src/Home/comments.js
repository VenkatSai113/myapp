...
const commentPosted=async()=>{
  ...
  const apiUrl="http://13.233.231.34:9000/comments"
  const options={
    method:"POST",
    headers:{
        ...
        <div className='comments-model-bg-container'>
            ...
            <SwipeableViews enableMouseEvents index={activeIndex} onChangeIndex={handleSlideChange}>
              {thumbnails.map(eachImage=>{
                <img src={`http://13.233.231.34:9000/${eachImage}`} className="descktop-comment-image" alt=""/>
                
                if(eachImage.split(".")[1]=="mp4"){
                  return(
                    <>
                     <div className='post-count-div'> <p className='post-count'>{`${activeIndex+1}/${thumbnails.length}`}</p></div>
                  <video  src={`http://13.233.231.34:9000/${eachImage}`} autoPlay loop muted  className="descktop-comment-image"/>
                  </>)
                }
                else{
                  return(
                  <>
                  <img alt="" src={`http://13.233.231.34:9000/${eachImage}`} className="descktop-comment-image" />
                  <div className='post-count-div'> <p className='post-count'>{`${activeIndex+1}/${thumbnails.length}`}</p></div>
                </>)
                }
