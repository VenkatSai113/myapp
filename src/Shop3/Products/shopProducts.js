...
function SelectedProducts(props) {
  ...
  return (
    ...
    <Card style={{ width: '18rem' ,margin:"7px" }} onClick={onClickProduct}>
      {splitedImages.length===1?<Card.Img variant="top" src={`https://venkatsai.onrender.com/${splitedImages}`} style={{height:"180px" ,width:"100%"}} /> :<SwipeableViews enableMouseEvents  index={activeIndex} onChangeIndex={handleSlideChange}>
      {splitedImages.map(eachImage=>
      <>
      <div className='post-count-div'> <p className='post-count'>{`${activeIndex+1}/${splitedImages.length}`}</p></div>
       <Card.Img variant="top" src={`https://venkatsai.onrender.com/${eachImage}`} style={{height:"180px" ,width:"100%"}} />  </>)}
      </SwipeableViews>}
      <Card.Body>
